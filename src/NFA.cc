 ...
 * @brief Contiene la implementación de la clase NFA
 * @version 0.1
 * @date 2022-11-23
 * 
 ...
 * @param in_file 
 */
void NFA::Generate_NFA (std::string& in_file) {
  ...
  std::string aux_string;
    ...
    for (long unsigned int i = 0; i < n_Symbol + 1; i++) {
      ifs >> aux_string;
      aux_chain += aux_string;
    }
    if (aux_chain[0]!= '~') {
      std::cout << "El caracter ~, no se encuentra en primer lugar, por favor corrijalo. " << "\n\n";
      exit(2);
    }
    ...
    if (initial_.get_Name() == "") {
      std::cout << "El estado inicial que ha seleccionado no existe, por favor corrijalo. " << "\n\n";
      exit(2);
    }
    ...
    int aux_cont = 0;
    for (long unsigned int i = 0; i < n_states; i++) 
      for (long unsigned int j = 0; j < n_final_states; j++) 
        if (aux_states[i].get_Name() == aux_final_states[j].get_Name()) 
          aux_cont++;
    if (aux_cont != n_final_states) {
      std::cout << "Ha habido algun error con la declaracion de los estados de aceptacion, por favor corrijelo. " << "\n\n";
      exit(2);
    }
    ...
    for (long unsigned int i = 0; i < n_transitions; i++) {
      ...
      for (long unsigned int j = 0; j < aux_states.size(); j++) 
        ...
        if (actual_exisist == false) {
          std::cout << "Uno de los de los estados declarado como inicial no existe, por favor corrijalo. " << "\n\n";
          exit(2);
        }
      ...
      for (long unsigned int j = 0; j < aux_states.size(); j++) {
        if (aux_states[j].get_Name() == aux_string) {
          ...
          std::set <char>::iterator it = alphabet_.get_Alphabet().begin(); 
          ...
          if (symbol_exisist == false) {
            std::cout << "Uno de los symbolos introducidos como transicion no esta declarado en el alfabeto, por favor corrijalo. " << "\n\n";
            exit(2);
          }
          ...
          if (next_exisist == false) {
            std::cout << "Uno de los de los estados declarado como transición no existe, por favor corrijalo. " << "\n\n";
            exit(2);
          }
          next_exisist = false;
 ...
 */
std::set <State> NFA::Epsilon_Clausure (std::set<State>& state) {

  bool not_in = true;
  std::stack <State> aux_stack;
  for (State est: state)
    aux_stack.push(est);
  std::set <State> e_clau = state;
 ...
 * @return Conjunto de estados
 */
std::set <State> NFA::Move (std::set<State>& state, char a) {
 ...
 */
void NFA::UnMark (std::set<State>& aux) {

  std::set <State> reverse_states;
  bool equal = false;
 ...
 */
bool NFA::Is_In (State aux, std::set<State> aux_set) const {
 ...
 * @return Verdadero si esta dentro del conjunto de estados del DFA
 */
bool NFA::Mark (State aux, std::set<State> aux_set) const {
 ...
 */
void NFA::Write_DFA (DFA dfa, std::string& file_out) {
  
  std::ofstream ofs (file_out, std::ios::in);

  if (ofs.fail()) {
    std::cout << "Error el documento que trata de usar para guardar no es valido, porfavor introduzca uno que si lo sea y reintentelo."
         << "\n\n";
    exit(2);
  } else {
 ...
 */
std::set <State> NFA::get_States (void) const {
  return states_;
}
 ...
 */
std::set <State> NFA::get_Acepted (void) const {
  return acept_;
}
 ...
 */
void NFA::set_Acepted (const std::set <State>& acept) {
  acept_ = acept;
}
 ...
 */
void NFA::Create_NFA_File (std::string& out_file) const {
